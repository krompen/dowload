...
async def init_db():
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        await db.execute("""CREATE TABLE IF NOT EXISTS user_chats (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, chat_id INTEGER, chat_title TEXT, topic_id INTEGER DEFAULT 0, is_system INTEGER DEFAULT 0)""")
...
def admin_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="➕ Добавить часы", callback_data="admin_add_hours"),
         InlineKeyboardButton(text="🗑 Мои автопосты", callback_data="admin_manage_posts")],
...
async def start(message: Message):
    ...
    await message.answer("🚀 <b>Autoposter Bot</b>\n\nАвтоматическая рассылка каждые 30 минут.", reply_markup=main_menu(message.from_user.id))
...
async def user_add_post_start(callback: CallbackQuery, state: FSMContext):
    # Проверка на 1 пост (кроме админа)
...
async def my_chats(callback: CallbackQuery):
    ...
    text = "📋 <b>Системные чаты</b>\n"
    system = [c[0] for c in chats if c[1] == 1]
    personal = [c[0] for c in chats if c[1] == 0]

    if system:
        text += "\n".join([f"• {c}" for c in system])
    else:
        text += "Нет системных чатов\n"

    text += "\n📋 <b>Мои чаты</b>\n"
    if personal:
        text += "\n".join([f"• {c}" for c in personal])
    else:
        text += "Нет личных чатов\n"
...
async def add_chat(message: Message):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        await db.commit()
    await message.answer("✅ Чат добавлен в личные!")
...
async def admin_broadcast(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("📢 Текст рассылки (будет в ЛС и чатах):")
...
async def admin_tickets(callback: CallbackQuery):
    ...
    text = "🎟 <b>Открытые тикеты</b>\n" + "\n".join([f"#{t[0]} | {t[1]}" for t in tickets]) if tickets else "Тикетов нет."
